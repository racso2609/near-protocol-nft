import { Worker, NEAR, NearAccount, tGas, BN } from "near-workspaces";
import anyTest, { TestFn } from "ava";
import { mint_more, nft_total_supply } from "./utils";
const WASM_FILE_PATH = process.argv[2];
...
test.beforeEach(async (t) => {
  ...
  const nft = await root.devDeploy(WASM_FILE_PATH, {
  ...
  const tokenReceiver = await root.createSubAccount("paul", {
    initialBalance: NEAR.parse("100 N").toJSON(),
  });

  const approvalReceiver = await root.createSubAccount("racso", {
    initialBalance: NEAR.parse("100 N").toJSON(),
  });
